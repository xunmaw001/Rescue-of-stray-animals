		...
		<view class="container">
			...
			<view class="detail-content">
				...
				<view class="detail-list-item"
					...
					<view class="lable"
						...
						时间：</view>
					<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
						{{detail.xinxifabuTime}}</view>
				</view>
				<view class="detail-list-item" v-if="detail.xinxifabuTypes == 3"
					:style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
					<view class="lable"
						:style='{"width":"auto","fontSize":"28rpx","color":"rgba(0, 102, 51, 1)","textAlign":"left"}'>
						信息状态：</view>
					<view class="text" :style='{"color":"rgba(0, 102, 51, 1)","textAlign":"center","fontSize":"24rpx"}'>
						{{detail.xinxiValue}}</view>
				</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.xinxifabuContent"></rich-text>
				</view>
			</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop" v-if="detail.xinxiTypes == 1 && detail.xinxifabuTypes == 3">
				<view style="text-align: left;display: flex;">
				</view>
				<view style="text-align: right;">
					<button @tap="onSubmitLingyang(detail.id)" style="margin-right: 10upx;min-width: 180upx;"
						class="btn-submit cu-btn round bg-red" v-if="user.id != detail.yonghuId"
						:style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[2],borderColor:btnColor[2]}">添加领养记录</button>
					<button @tap="onUpdateXinxiTypes(detail)" style="margin-right: 10upx;min-width: 180upx;"
						class="btn-submit cu-btn round bg-red" v-if="user.id == detail.yonghuId"
						:style="{width:'auto',borderRadius:'40rpx',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[2],borderColor:btnColor[2]}">改为已完成</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('xinxifabuLiuyanState')
			if (cleanType) {
				uni.removeStorageSync('xinxifabuLiuyanState')
				this.mescroll.num = 1
		...
		methods: {
			// 获取详情
			async init() {
				let res = await this.$api.info('xinxifabu', this.id);
				this.detail = res.data;
				// 轮播图片
				this.swiperList = this.detail.xinxifabuPhoto ? this.detail.xinxifabuPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('xinxifabuLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					xinxifabuId: this.id
				});
			...
			onUpdateXinxiTypes(detail){
				detail.xinxiTypes = 2
				this.$api.update('xinxifabu', detail);
				uni.redirectTo({
				    url: '/pages/xinxifabu/detail?id='+detail.id
				});
			},
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../xinxifabuLiuyan/add-or-update?xinxifabuId=${this.id}`)
			},
		}
	}
</script>
