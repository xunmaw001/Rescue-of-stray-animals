<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
		...
		<view class="search-form round">
			...
			<input v-model="searchForm.xinxifabuName" type="text" placeholder="信息标题"></input>
		</view>
		<view class="action">
			<button @tap="search"
	...
	<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
		<view @tap="onDetailTap(product)" class="uni-product"
			...
			<view class="uni-product-title"
				...
				{{product.xinxifabuName}}</view>
			<view class="image-view">
				<image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
					class="uni-product-image" :src="product.xinxifabuPhoto?product.xinxifabuPhoto.split(',')[0]:''"></image>
			</view>
			<view style="display: flex;justify-content: space-between;"> 
				<text v-if="isAuth('xinxifabu','修改') && user.id == product.yonghuId" class="cuIcon-edit"
					@click.stop="onUpdateTap(product.id)">修改</text>
				<text v-if="isAuth('xinxifabu','删除') && user.id == product.yonghuId" class="cuIcon-delete"
	...
	<button
		...
		v-if="isAuth('xinxifabu','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			...
			let res = await _this.$api.page('dictionary', {
				page: 1,
				limit: 100,
				dicCode:'xinxifabu_types'
			});
		...
		methods: {
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xinxifabu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.xinxifabuName) {
					searchForm['xinxifabuName'] = '%' + this.searchForm.xinxifabuName + '%'
				}
				let res = await this.$api.list(`xinxifabu`, searchForm);
				// 如果是第一页数据置空
